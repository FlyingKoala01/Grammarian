      ...
      <label className="flex flex-col gap-2 text-sm font-medium text-slate-700">
        ...
        <div className="relative">
          {isFocused && toneSuggestionState ? (
            <div className="absolute inset-x-0 top-0 z-10 flex -translate-y-[calc(100%+0.5rem)] flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-[0_12px_30px_rgba(15,23,42,0.08)] sm:left-4 sm:right-auto sm:w-auto">
              ...
              <div className="flex flex-wrap gap-1">
                {toneSuggestionState.options.map((option, index) => (
                  <button
                    className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-sm text-slate-700 transition hover:border-amber-500 hover:text-amber-800"
                    disabled={disabled}
                    key={`${option}-${index}`}
                    onPointerDown={(event) => {
          ...
          <input
            className="w-full rounded-[1.25rem] border border-slate-200 bg-slate-50 px-4 py-3 text-base outline-none transition focus:border-amber-500"
            disabled={disabled}
